...
private:
	...
	void DrawVertBar();
	//void MoveVertPanes(CPoint SplitPoint);		// Moved to public operations
	...
	void DrawHorizBar();
	//void MoveHorizPanes(CPoint SplitPoint);		// Moved to public operations
...
public:
	...
	// liush79 추가: MoveVertPanes 이나 MoveHorizPanes 를 사용한 후에 이 함수를 호출해줘야 다이얼로그가 깨지지 않음.
	void SetBottomPaneHeight(LONG height) { m_nBottomPaneHeight = height; }
	void SetLeftPaneWidth(LONG width) { m_nLeftPaneWidth = width; }
